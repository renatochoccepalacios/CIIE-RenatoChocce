...
class UsuarioDAL extends main
{
    ...
    public function login($dni, $password)
    {
        ...
        $query = sprintf(
            "SELECT idTipoCuenta FROM usuarios WHERE dni='%s' AND password='%s' AND idEstado='1'",
            $dni,
            $password
        );
    ...
    {
        $query = sprintf(
            "UPDATE usuarios SET nombre='%s', apellido='%s', telefono='%s', correo='%s' WHERE dni='%s'",
            $usuario->getNombre(),
            $usuario->getApellido(),
            $usuario->getTelefono(),
            $usuario->getCorreo(),
    ...
    {
        $query = sprintf(
            "UPDATE usuarios SET idEstado = 2 WHERE dni = '%s'",
            $dni
        );
    ...
    {
        $query = sprintf(
            "UPDATE usuarios SET idEstado = 1 WHERE dni = '%s'",
            $dni
        );
